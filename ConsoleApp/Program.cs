...
class Program {
    ...
    ///<summary>
    ///Метод получения списков из dll библиотеки классов
    ///</summary>
    private static void LoadLessonsList()
    {
        Assembly asm = Assembly.LoadFrom("LessonsLib.dll");
        //создание сборки из библиотеки классов
        Type[] types = asm.GetTypes();
        //выгрузка классов в массив
        foreach (Type type in types) 
        {
            //перебираем классы и  интерфейсы
            if ((type.IsInterface == false) && (type.IsAbstract == false))
            //не добавляем абстрактные классы и интерфейс
            {
                foreach (var method in type.GetMethods())
                //перебираем методы класса
                {
                    if (method.ToString().Contains("Demo"))
                    //если среди методов класса содержится Demo
                    {
                        ILabs labs = (ILabs)Activator.CreateInstance(type);
                        _labs.Add(labs);
                        //погружаем этот класс в список лабораторных
                    }
                }
            }
        }
